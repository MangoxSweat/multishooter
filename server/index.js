...
const Dragonfly = require('dragonfly');

const app = express();
const server = http.createServer(app);
const wss = new WebSocketServer({ server });
const dragonfly = new Dragonfly({ host: 'localhost', port: 6379 });
...
wss.on('connection', (ws) => {
	...
	ws.on('message', (message) => {
		...
		switch (data.type) {
			case 'move':
				dragonfly.set(`player:${playerId}`, JSON.stringify(playerData));
			...
			case 'shoot':
				...
				dragonfly.set(`projectile:${projectileId}`, JSON.stringify(projectile));
